...
export const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div 
        className={`fixed inset-y-0 left-0 z-40 w-64 bg-indigo-700 text-white transition-transform duration-300 ease-in-out transform ${
          sidebarOpen ? 'translate-x-0' : '-translate-x-full'
        } lg:translate-x-0 lg:static lg:inset-0 lg:h-full flex flex-col`}
      >
        <div className="flex items-center justify-center h-16 bg-gradient-to-r from-indigo-800 to-indigo-700">
          <h1 className="text-xl font-bold tracking-tight">Transellia Admin</h1>
        </div>
            ...
            {navigation.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`${
                    isActive
                      ? 'bg-indigo-800/80 text-white border-l-4 border-white/70'
                      : 'text-indigo-100 hover:bg-indigo-600/70 hover:text-white border-l-4 border-transparent'
